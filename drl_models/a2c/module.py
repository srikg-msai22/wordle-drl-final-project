...
import drl_models.a2c
import env.state
from drl_models.a2c.agent import ActorCriticAgent
from drl_models.a2c.experience import ExperienceSourceDataset, Experience
...
class AdvantageActorCritic(LightningModule):
    ...
    def __init__(
        ...
        self.net = drl_models.a2c.construct(
    ...
    def train_batch(self) -> Iterator[Tuple[np.ndarray, int, Tensor]]:
        ...
        while True:
            ...
            for _ in range(self.hparams.batch_size):
                ...
                if env.state.remaining_steps(self.state) == 1 and self._cheat_word:
                ...
                if done:
                    if action == self.env.goal_word:
                        self._winning_steps += self.env.max_turns - env.state.remaining_steps(self.state)
                        self._wins += 1
