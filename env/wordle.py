...
class WordleEnvBase(gym.Env):
    ...
    def __init__(self, words: List[str],
        ...
        self.observation_space = spaces.MultiDiscrete(env.state.get_nvec(self.max_turns))

        self.done = True
        self.goal_word: int = -1

        self.state: env.state.WordleState = None
        self.state_updater = env.state.update
        if self.mask_based_state_updates:
            self.state_updater = env.state.update_mask
    ...
    def reset(self, seed: Optional[int] = None):
        self.state = env.state.new(self.max_turns)
        self.done = False
