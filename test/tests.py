...
import env.wordle as env_wordle
import env.state as env_state
...
def test_win_reward(wordleEnv):
    ...
    assert env_state.remaining_steps(new_state) == wordleEnv.max_turns-1
    assert not done
    assert not wordleEnv.done
    assert reward == 0

    new_state, reward, done, _ = wordleEnv.step(goal)
    assert env_state.remaining_steps(new_state) == wordleEnv.max_turns-2
    assert done
    assert wordleEnv.done
    assert reward == env_wordle.REWARD
...
def test_win_reward_6(wordleEnv):
    ...
    assert wordleEnv.max_turns - env_state.remaining_steps(new_state) == 6
    assert done
    assert wordleEnv.done
    assert reward == env_wordle.REWARD
...
def test_lose_reward(wordleEnv):
    ...
    for i in range(1, wordleEnv.max_turns):
        ...
        assert env_state.remaining_steps(new_state) == wordleEnv.max_turns-i
        assert not done
        assert not wordleEnv.done
        assert reward == 0

    new_state, reward, done, _ = wordleEnv.step((goal + wordleEnv.max_turns) % len(wordleEnv.words))
    assert env_state.remaining_steps(new_state) == 0
    assert done
    assert wordleEnv.done
    assert reward == -env_wordle.REWARD
...
def test_step(wordleEnv):
    ...
    assert env_state.remaining_steps(cur_state) == wordleEnv.max_turns
    assert env_state.remaining_steps(new_state) == wordleEnv.max_turns-1
    ...
    cur_state = wordleEnv.state
    new_state, reward, done, _ = wordleEnv.step(1)
    assert env_state.remaining_steps(cur_state) == wordleEnv.max_turns-1
    assert env_state.remaining_steps(new_state) == wordleEnv.max_turns-2
    # Expect B to be all 1,0,0
    offset = 1 + 26 + 3 * 5 * (ord('B') - ord('A'))
    assert tuple(new_state[offset:offset+15]) == tuple([1, 0, 0]*5)
    ...
    assert env_state.remaining_steps(new_state) == wordleEnv.max_turns-3
    # Expect B to be all 1,0,0
    offset = 1+26+3*5*(ord('B')-ord('A'))
    assert tuple(new_state[offset:offset+15]) == tuple([1, 0, 0]*5)
    ...
    assert env_state.remaining_steps(new_state) == wordleEnv.max_turns-4
    assert done
    assert wordleEnv.done
    assert reward == env_wordle.REWARD
